...
contains
  ...
  function integer_to_string(x, zero_pad_width) result(res)

    integer, intent(in) :: x
    integer, intent(in), optional :: zero_pad_width
    character(:), allocatable :: res

    character(10) format_str
    character(range(x)+2) tmp

    if (present(zero_pad_width)) then
      write(format_str, '("(I", I0, ".", I0, ")")') zero_pad_width, zero_pad_width
      write(tmp, format_str) x
    else
      write(tmp, '(I0)') x
    end if
    res = tmp
