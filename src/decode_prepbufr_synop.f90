...
contains
  ...
  subroutine decode(file_path, stations, records)
    ...
    do while (ireadmg(10, subset, idate) == 0) ! ireadmg returns mnemonic in subset, and copies message into internal arrays.
      ...
      do while (ireadsb(10) == 0) ! ireadsb copies one subset into internal arrays.
        ...
        if (record%PS == real_missing_value) record%PS = prepbufr_raw(obs(1,1,:), pc(1,1,:))
        if (record%T  == real_missing_value) record%T  = prepbufr_raw(obs(2,1,:), pc(2,1,:))
        if (record%TD == real_missing_value) record%TD = prepbufr_raw(obs(3,1,:))
        if (record%WS == real_missing_value) then
          u = prepbufr_raw(obs(4,1,:), pc(4,1,:))
          v = prepbufr_raw(obs(5,1,:), pc(4,1,:))
          record%WS = merge(real_missing_value, sqrt(u**2 + v**2), u == real_missing_value)
          record%WD = merge(real_missing_value, wind_direction(u, v), u == real_missing_value)
        end if
        if (record%TP01 == real_missing_value) record%TP01 = prepbufr_raw(obs(6,1,:))
        if (record%TP03 == real_missing_value) record%TP03 = prepbufr_raw(obs(7,1,:))
        if (record%TP06 == real_missing_value) record%TP06 = prepbufr_raw(obs(8,1,:))
        if (record%TP12 == real_missing_value) record%TP12 = prepbufr_raw(obs(9,1,:))
        if (record%TP24 == real_missing_value) record%TP24 = prepbufr_raw(obs(10,1,:))
  ...
  subroutine write_odb(stations, records)
    ...
    call odbql_prepare_v2(odb_db, 'CREATE TABLE synop AS (' // &
      'station_name STRING, lon REAL, lat REAL, z REAL, time STRING, T REAL, TD REAL, WS REAL, WD REAL, PS REAL) ON "' // trim(odb_file_name) // '";', &
      -1, odb_stmt, odb_unparsed_sql)
    call odbql_prepare_v2(odb_db, 'INSERT INTO synop (' // &
      'station_name, lon, lat, z, time, T, TD, WS, WD, PS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);', &
      -1, odb_stmt, odb_unparsed_sql)
    ...
    do while (.not. record_iterator%ended())
      ...
      type is (synop_record_type)
        ...
        call odbql_bind_double(odb_stmt,  7, dble(record%TD))
        call odbql_bind_double(odb_stmt,  8, dble(record%WS))
        call odbql_bind_double(odb_stmt,  9, dble(record%WD))
        call odbql_bind_double(odb_stmt, 10, dble(record%PS))
