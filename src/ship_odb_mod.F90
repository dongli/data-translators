  ...
  subroutine ship_odb_write(file_path, ships, records)
    ...
    do while (.not. record_iterator%ended())
      ...
      type is (ship_record_type)
        ...
        col = col + 1; if (.not. is_missing(record%ship_pressure))          call odbql_bind_double(odb_stmt, col, dble(record%ship_pressure))
        col = col + 1; if (.not. is_missing(record%ship_air_temperature))   call odbql_bind_double(odb_stmt, col, dble(record%ship_air_temperature))
        col = col + 1; if (.not. is_missing(record%ship_sea_temperature))   call odbql_bind_double(odb_stmt, col, dble(record%ship_sea_temperature))
        col = col + 1; if (.not. is_missing(record%ship_dewpoint))          call odbql_bind_double(odb_stmt, col, dble(record%ship_dewpoint))
        col = col + 1; if (.not. is_missing(record%ship_relative_humidity)) call odbql_bind_double(odb_stmt, col, dble(record%ship_relative_humidity))
        col = col + 1; if (.not. is_missing(record%ship_specific_humidity)) call odbql_bind_double(odb_stmt, col, dble(record%ship_specific_humidity))
        col = col + 1; if (.not. is_missing(record%ship_wind_u))            call odbql_bind_double(odb_stmt, col, dble(record%ship_wind_u))
        col = col + 1; if (.not. is_missing(record%ship_wind_v))            call odbql_bind_double(odb_stmt, col, dble(record%ship_wind_v))
        col = col + 1; if (.not. is_missing(record%ship_cloud_cover))       call odbql_bind_double(odb_stmt, col, dble(record%ship_cloud_cover))
