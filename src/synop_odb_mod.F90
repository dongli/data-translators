...
contains
  ...
  subroutine synop_odb_write(file_path, stations, records)
    ...
    call odbql_prepare_v2(odb_db, 'CREATE TABLE synop AS (' // &
      ...
      'dewpoint REAL, '                    // & ! 10
      'relative_humidity REAL, '           // & ! 11
      'relative_humidity_qc INTEGER, '     // & ! 12
      'wind_u REAL, '                      // & ! 13
      'wind_v REAL, '                      // & ! 14
      'wind_qc INTEGER, '                  // & ! 15
      'pressure REAL, '                    // & ! 16
      'pressure_qc INTEGER '               // & ! 17
    ...
    do while (.not. record_iterator%ended())
      ...
      type is (synop_record_type)
        ...
        call odbql_bind_double(odb_stmt, 11, dble(record%sfc_relative_humidity))
        call odbql_bind_int   (odb_stmt, 12, record%sfc_relative_humidity_qc)
        call odbql_bind_double(odb_stmt, 13, dble(record%sfc_wind_u))
        call odbql_bind_double(odb_stmt, 14, dble(record%sfc_wind_v))
        call odbql_bind_int   (odb_stmt, 15, record%sfc_wind_qc)
        call odbql_bind_double(odb_stmt, 16, dble(record%sfc_pressure))
        call odbql_bind_int   (odb_stmt, 17, record%sfc_pressure_qc)
